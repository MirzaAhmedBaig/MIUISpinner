...
import org.avantari.dhyana.util.dpToPx

class LoadingView : View {
    internal var TAG = LoadingView::class.java.simpleName
    ...
    private var progressColor = Color.parseColor("#8cdbfb")
...
//                }
    ...
    private fun refresh() {
        ...
        arcPath!!.addCircle((getWidth() / 2).toFloat(), (getHeight() / 2).toFloat(), (getWidth() / 4).toFloat(), Path.Direction.CW)
        val center_x: Float = (getWidth() / 2).toFloat()
        val center_y: Float = (getHeight() / 2).toFloat()
        mCircleRectF = RectF()

        mCircleRectF!!.set(center_x - getWidth() / 4,
                center_y - getWidth() / 4,
                center_x + getWidth() / 4,
                center_y + getWidth() / 4)
